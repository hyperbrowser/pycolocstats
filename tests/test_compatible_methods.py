...
def tracks():
    return [
            ...
            TrackFile(os.path.join(TEST_RESOURCES_DIR, 'test_track5.bed'), 't5'),
            TrackFile(os.path.join(TEST_RESOURCES_DIR, 'test_track6.bed'), 't6'),
            ]
...
class TestMethods(TestMethodsBase):
    ...
    def test_goShifter_genome_hg38(self, tracks):
        selectionValues = [[('setGenomeName', u'hg38')],
                           [('setRuntimeMode', u'quick')]]
    ...
    def test_adv_proximity(self,chrLenFile, queryTrack, refTracks, tracks):
        selectionValues = [[('setRestrictedAnalysisUniverse', None)],
                           [('setColocMeasure', ColocMeasureProximity(None, None))],
                           [('preserveClumping', False)],
                           [('setGenomeName', u'hg19')],
                           [('setRuntimeMode', u'quick')],
                           [('setChromLenFileName', chrLenFile)]]
        workingMethodObjects = WorkingMethodObjectParser(queryTrack, refTracks, selectionValues,
                                                         ALL_PYCOLOCSTATS_METHOD_CLASSES).getWorkingMethodObjects()
